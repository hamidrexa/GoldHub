...
export default function TransactionBox({ dict, lang, className }: Props) {
    // ** Hooks
    const {
        price,
    ...
    const [transactionMode, setTransactionMode] = useState<any>(
        type === 'sell' ? 'sell' : 'buy'
    );
        ...
        if (value) {
            setTomanEq(
                formatWithCommas(
                    currency(
                        roundNumber(
                            parseInt(mgram) * (parseInt(value) / 1000),
                            0
                        ),
                        'tse',
                        'fa'
    ...
    const onPaymentClick = async () => {
        ...
        if (buyWithWallet && transactionMode === 'buy') {
            ...
            if (Number(sliderValue) < 100000)
                return toast.warning(
                    'حداقل مبلغ پرداختی ۱۰۰ هزار تومان میباشد.'
                );
            if (Number(sliderValue) > 50000000)
                return toast.warning(
                    'حداکثر مبلغ پرداختی ۵۰ میلیون تومان میباشد.'
                );
            setLoading(true);
            await exchange(null, {
        ...
        if (transactionMode === 'buy') {
            ...
            if (Number(rial) < 100000)
                return toast.warning(
                    'حداقل مبلغ پرداختی ۱۰۰ هزار تومان میباشد.'
                );
            if (Number(rial) > 50000000)
                return toast.warning(
                    'حداکثر مبلغ پرداختی ۵۰ میلیون تومان میباشد.'
                );
            setLoading(true);
    ...
    return (
        <div className="flex w-full justify-center">
            <div
                className={cn(
                    'relative flex w-full flex-col gap-8 rounded-md border-gray-400 bg-white px-4 py-6 text-black md:max-w-2xl md:border',
                    className
                )}
            >
                {(loading || priceIsLoading) && (
                    <div className="absolute inset-0 flex items-center justify-center">
                        <div className="z-100000 absolute inset-0 z-10 bg-white opacity-50"></div>
                ...
                <div className="flex flex-row items-center justify-between">
                    <div className="flex flex-row gap-[10px]">
                        ...
                        <h2 className="text-[22px] font-bold">معامله</h2>
                    </div>
                ...
                <div className="flex w-full flex-row justify-center">
                    <RadioGroup
                        ...
                        className="flex w-[70%] w-full max-w-[400px] items-center gap-[0px] rounded-lg border border-gray-400 p-1"
                    >
                        <div className="w-full overflow-hidden">
                            ...
                            <Label
                                htmlFor={'buy'}
                                className="flex w-full cursor-pointer flex-col items-center justify-between text-nowrap rounded-md border-transparent bg-transparent px-1 py-2  peer-data-[state=checked]:bg-[#CA8A04] peer-data-[state=checked]:font-black peer-data-[state=checked]:text-white [&:has([data-state=checked])]:border-neutral-100 [&:has([data-state=checked])]:bg-[#CA8A04] [&:has([data-state=checked])]:font-black"
                            >
                                خرید طلا
                            </Label>
                        </div>
                    ...
                    <div className="flex w-full flex-col items-end justify-center gap-3">
                        <div className="flex w-full items-center justify-between">
                            ...
                            <div className="flex items-center justify-end gap-2">
                                <Label>خرید از موجودی</Label>
                                <Switch
                                    className="data-[state=checked]:bg-[#0C0E3C relative h-[23px] w-[42px] rounded-full bg-[#0C0E3C] outline-none"
                        ...
                        {buyWithWallet && (
                            <div className="flex w-full flex-col justify-center gap-6  md:items-center">
                                <NewRangeSlider
                                    min={0}
                                    disabled={
                                        Number(wallet?.balance?.irt_balance) <=
                                        0
                                    }
                                    max={
                                        Number(wallet?.balance?.irt_balance) ||
                ...
                <div
                    className={`flex w-full gap-8 ${transactionMode === 'sell' ? 'flex-col-reverse' : ' flex-col'}`}
                >
                    {(!buyWithWallet || transactionMode === 'sell') && (
                    ...
                    <div className="flex w-full flex-col gap-[12px]">
                        <div className="flex w-full flex-row items-center justify-between">
                            ...
                            {transactionMode === 'sell' && (
                                <Label className="text-[#07BB61]">
                                    موجودی طلا:{' '}
                                    {roundNumber(
                                        Number(wallet?.balance?.gold_amount) *
                                            1000,
                                        4
                                    )}{' '}
                                    میلی گرم
                        ...
                        <div
                            style={{
                                visibility:
                                    mGramEq?.length > 0 ? 'visible' : 'hidden',
                            }}
                        ...
                        >
                            ...
                            <span className="mx-1">
                                {' '}
                                {Number(
                                    JSON.stringify(mGramEq)?.replace(/\D/g, '')
                                ) / 1000}
                            </span>
                            گرم طلای ۱۸ عیار
                        </div>
                ...
                <div className="flex w-full flex-col gap-[12px]">
                    ...
                    <div className="flex w-full items-center justify-center">
                        <div className={`${isValidating ? 'blur-sm' : ''}`}>
                            {`1 گرم طلا`} ={' '}
                            {`${currency(transactionMode === 'buy' ? price?.buy_price_irt : price?.sell_price_irt, 'tse', lang)} تومان`}
                        </div>
                        <div
                            className={`${isValidating ? 'opacity-1' : 'opacity-0'}`}
                        >
                            {
                                <Spinner
                                    className={`mx-2 h-[15px] w-[15px] opacity-[${isValidating ? '2' : '0'}]`}
                                />
                            }
                        </div>
                    </div>
                ...
                <div>
                    <div className="flex flex-row items-center justify-between">
                        <Label className="font-black">کارمزد</Label>
                        <div className="flex flex-row items-center gap-2 font-black text-[#5A5C83]">
                            {currency(0, 'tse', lang)}{' '}
                            {transactionMode === 'buy' ? 'تومان' : 'میلی گرم'}{' '}
                            <Icons.info stroke="#2228A9" />
                        </div>
                    </div>
                </div>
                {/* <div>
                ...
                <Button
                    className="sticky bottom-[80px] md:bottom-[5px]"
                    onClick={() => {
                        toast.info('امکان خرید طلا تا اطلاع ثانوی وجود ندارد.');
                    }}
                >
                    ادامه
                </Button>
