...
// Calculate progress percentage based on status
    ...
    const statusCounts = {
        ...
        active: filteredHistory.filter(o => normalizeStatus(o.status) === 'active').length,
        completed: filteredHistory.filter(o => normalizeStatus(o.status) === 'completed').length,
    };
                                ...
                                <div className="flex flex-col sm:flex-row items-start sm:justify-between gap-4">
                                    ...
                                    <div className="flex-1 space-y-3">
                                        ...
                                        <div className="text-sm text-muted-foreground space-y-1">
                                            <p>{dict.marketplace.buyer.ordersPage.card.supplier} {order.items[0]?.product.supplier.username.includes('Premium') ? 'Premium Gold Co.' : 'Luxury Jewelers Inc.'}</p>
                                    ...
                                    <div className="text-right space-y-3">
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">
                                                {order.items.length} {dict.marketplace.buyer.ordersPage.card.itemsSuffix}
                                            </p>
                                        </div>
                                ...
                                <TableBody>
                                    {filteredHistory.map((order) => (
                                        <TableRow key={order.id}>
                                            ...
                                            <TableCell>{order.items.length} {dict.marketplace.buyer.ordersPage.card.itemsSuffix}</TableCell>
