...
export function OrderPageWrapper({dict,lang,orderId}){
    const { history = [],isLoading,error } =  useOrdersHistory(orderId);
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <StatusBadge status={order.status.toLowerCase()} dict={dict} />
                </div>
            </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>{dict.marketplace.buyer.orderDetailPage.orderItems} ({order.items.length})</CardTitle>
                </CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead
                                    className="text-right">{dict.marketplace.buyer.orderDetailPage.table.subtotal}</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                    ...
                    <div className="mt-6 space-y-2 max-w-sm ml-auto">
                        <div className="flex justify-between text-sm">
                            <span
                                className="text-muted-foreground">{dict.marketplace.buyer.orderDetailPage.summary.subtotal}</span>
                            <span>${order.total_price.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span
                                className="text-muted-foreground">{dict.marketplace.buyer.orderDetailPage.summary.tax}</span>
                            <span>${order?.total_price.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span
                                className="text-muted-foreground">{dict.marketplace.buyer.orderDetailPage.summary.shipping}</span>
                            <span>${order?.total_price.toFixed(2)}</span>
                        </div>
                        <Separator />
