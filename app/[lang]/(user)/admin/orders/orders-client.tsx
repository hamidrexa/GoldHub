...
export default function OrdersClient({ dict }: OrdersClientProps) {
    const [searchQuery, setSearchQuery] = useState('');
    const [activeTab, setActiveTab] = useState('all');

    const getStatusBadge = (status: Order['status']) => {
        const badges = {
            confirmed: { variant: 'default' as const, label: dict.marketplace.admin.ordersPage.status.confirmed, className: 'bg-yellow-100 text-yellow-800 hover:bg-yellow-100' },
            shipped: { variant: 'default' as const, label: dict.marketplace.admin.ordersPage.status.shipped, className: 'bg-blue-100 text-blue-800 hover:bg-blue-100' },
            pending_supplier: { variant: 'default' as const, label: dict.marketplace.admin.ordersPage.status.pendingSupplier, className: 'bg-orange-100 text-orange-800 hover:bg-orange-100' },
            closed: { variant: 'default' as const, label: dict.marketplace.admin.ordersPage.status.closed, className: 'bg-green-100 text-green-800 hover:bg-green-100' },
            cancelled: { variant: 'default' as const, label: dict.marketplace.admin.ordersPage.status.cancelled, className: 'bg-red-100 text-red-800 hover:bg-red-100' },
        };
        ...
        <div className="space-y-6">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-transparent border-b w-full justify-start rounded-none h-auto p-0 space-x-4">
                    ...
                    >
                        {dict.marketplace.admin.ordersPage.tabs.all}
                    </TabsTrigger>
                    <TabsTrigger
                        value="pending"
                        className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-3"
                    >
                        {dict.marketplace.admin.ordersPage.tabs.pending} ({pendingCount})
                    </TabsTrigger>
                    <TabsTrigger
                        value="active"
                        className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-3"
                    >
                        {dict.marketplace.admin.ordersPage.tabs.active} ({activeCount})
                    </TabsTrigger>
                    <TabsTrigger
                        value="closed"
                        className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-3"
                    >
                        {dict.marketplace.admin.ordersPage.tabs.closed}
                    </TabsTrigger>
                </TabsList>
